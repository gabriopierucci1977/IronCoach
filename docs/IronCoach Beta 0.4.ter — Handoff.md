# IronCoach Beta 0.4.ter — Handoff

**Data handoff:** 28 agosto 2026
**Branch:** `feature/beta-0.4-decision-memory`
**Ultimo commit codice prima dell'handoff:** `1cf5ef9`
**Commit:** `feat: add Garmin recovery observations`
...
- ultimo commit remoto codice: `1cf5ef9`;
- suite completa:
  - **500 passed**
  - **5 skipped**
  - circa 1.8-1.9 s;
...
`get_latest_nutrition()` restituisce assenza di dato.

Non è stata scelta Garmin come fonte nutrizione.

Principio:

> non usare Garmin come sorgente nutrizionale completa senza un contratto dati affidabile.

Nutrition rimane quindi un prossimo blocco da progettare separatamente.

---

## 36. Performance

`Performance Log` non contiene uno storico reale valido.

`get_performance_history()` ha un fallback storico già esistente che, quando la tabella è vuota, restituisce un elemento costruito dal profilo atleta con:
...
- non ha una data;
- non rappresenta uno storico temporale reale;
- non è sufficiente per un trend Performance.
...
quando mancano osservazioni confrontabili nel tempo.
...
Il prossimo blocco naturale è definire il percorso dati reale per:
...
- quali dati servono realmente al DecisionEngine;
- quale fonte può fornirli in modo affidabile;
- quali campi sono manuali;
- quali possono essere automatizzati.

Non assumere Garmin come fonte completa.

### Performance

Prima separare:

- metriche profilo statiche;
- metriche temporali;
- performance delle attività;
- eventuali benchmark/FTP/CSS/VO₂max con data.

Poi decidere quale storico reale può alimentare:

`PerformanceHistory`

senza inventare trend.
...
- Garmin archive live: **3896 attività**;
...
- Garmin Recovery adapter: implementato;
- Garmin Recovery archive: implementato;
- Garmin Recovery sync: implementato;
- Garmin Recovery automatic best-effort: implementato;
- Body Battery separata da readiness;
- Recovery Garmin osservativo non altera ancora decisioni;
- Nutrition reale: ancora da progettare;
- Performance storico reale: ancora da progettare;
- prossimo step: Nutrition e Performance, una sorgente alla volta.
