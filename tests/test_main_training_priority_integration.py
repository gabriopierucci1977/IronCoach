...
def test_main_attaches_priority_aware_modified_workout(
    ...
    class FakeDecisionWriter:
        def __init__(self, client):
            captured["writer_client"] = client

        def save(self, saved_decision):
            captured["saved_decision"] = saved_decision
    ...
    saved_decision = captured["saved_decision"]
    workout = saved_decision["modified_workout"]

    assert captured["coach_context"] is context
    assert captured["report_context"] is context
    assert captured["report_decision"] is saved_decision
